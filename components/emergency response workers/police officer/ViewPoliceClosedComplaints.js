...
export default function ViewPoliceClosedComplaints({ navigation}) {
  ...
  const [cid,setcid] = useState('');   
    ...
    useEffect(()=>{
        ...
        .where('status', '==', false )
        .where('closedby','==', auth().currentUser.uid )
        .where("dept.name",'==','Police')
        .get()
        .then(querySnapshot => {
    ...
    <View >
        ...
        <TouchableOpacity
         style={{
           ...
           onPress={() => navigation.navigate('HomePoliceOfficer')}
           >
